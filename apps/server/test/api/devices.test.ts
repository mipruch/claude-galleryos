...
async function req<T>(path: string): Promise<{ status: number; body: T }> {
...
describe("GET /devices?role_id=", () => {
  test("no role_id returns everything, unchanged from before", async () => {
    const { status, body } = await req<{ id: string }[]>("/api/v1/devices");
    expect(status).toBe(200);
    expect(body.map((d) => d.id)).toEqual(["d1", "d2", "d3"]);
  });

  test("an admin role sees everything", async () => {
    const { body } = await req<{ id: string }[]>("/api/v1/devices?role_id=r-admin");
    expect(body.map((d) => d.id)).toEqual(["d1", "d2", "d3"]);
  });

  test("a scoped role only sees its role_devices", async () => {
    const { body } = await req<{ id: string }[]>("/api/v1/devices?role_id=r-scoped");
    expect(body.map((d) => d.id)).toEqual(["d2"]);
  });

  test("a role with no devices granted sees nothing", async () => {
    const { body } = await req<{ id: string }[]>("/api/v1/devices?role_id=r-empty");
    expect(body).toEqual([]);
  });

  test("an unknown role_id is treated like no filter (fails open)", async () => {
    const { body } = await req<{ id: string }[]>("/api/v1/devices?role_id=does-not-exist");
    expect(body.map((d) => d.id)).toEqual(["d1", "d2", "d3"]);
  });
});
...
describe("GET /devices/live?role_id=", () => {
  test("scopes the snapshot map to the role's devices", async () => {
    const { status, body } = await req<Record<string, unknown>>("/api/v1/devices/live?role_id=r-scoped");
    expect(status).toBe(200);
    expect(Object.keys(body)).toEqual(["d2"]);
  });

  test("no role_id returns the full snapshot", async () => {
    const { body } = await req<Record<string, unknown>>("/api/v1/devices/live");
