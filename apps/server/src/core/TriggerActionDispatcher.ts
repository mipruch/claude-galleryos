...
export class TriggerActionDispatcher {
	...
	async dispatchAll(
		...
		for (const action of actions)
			outcomes.push(
				await this.dispatch(action, source, sourceDetail, template)
			);
		return outcomes;
	}
	...
	async dispatch(
		...
		const params = template
			...
			: action.params;
		this.log.info("dispatching trigger action", {
			...base,
			params,
			source,
			sourceDetail,
		...
		try {
			switch (action.targetType) {
				case "scene.execute": {
					const result = await this.opts.sceneEngine.startScene(
						action.targetId,
						source,
						{sourceDetail}
					);
					...
					return {
						...base,
						ok: true,
						detail: `execution ${result.executionId}`,
					};
				}
				case "device.command": {
					if (!action.targetCommand) {
						return {
							...base,
							ok: false,
							detail: "device.command action has no targetCommand",
						};
					}
					const result = await this.opts.deviceManager.execute(
						action.targetId,
						action.targetCommand,
						params
					);
					this.log[result.success ? "info" : "warn"](
						"trigger action ran command",
						{
							triggerActionId: action.id,
			...
			this.log.warn("trigger action dispatch failed", {
				triggerActionId: action.id,
				error: errMsg(err),
			});
