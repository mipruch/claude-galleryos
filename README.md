...
|-------------------------------|------|-----------------------------------------------------------------------------------------------------|
|**Node.js**                    |20 LTS|Event-driven architektura, skvělý ekosystém síťových protokolů                                       |
|**TypeScript**                 |5.x   |Typová bezpečnost, IDE podpora, srozumitelnost kódu                                                  |
|**Fastify**                    |4.x   |Výkonný HTTP server, WebSocket podpora přes `@fastify/websocket`, JSON Schema validace out-of-the-box|
|**Socket.io**                  |4.x   |Real-time WebSocket s fallbackem, rooms pro broadcast skupinám klientů                               |
|**node-cron**                  |3.x   |CRON plánovač, dynamicky konfigurovatelný za běhu                                                    |
|**EventEmitter** (Node.js core)|—     |Interní Event Bus — synchronní, bez externích závislostí pro základní verzi                          |
|**Ajv**                        |8.x   |JSON Schema validace params driverů a API vstupů                                                     |
|**Winston**                    |3.x   |Strukturované logování, transport do souboru + InfluxDB                                              |
|**node-osc**                   |4.x   |OSC přijímač (UDP server)                                                                            |

### Databáze

|Technologie               |Použití                                                                                          |
|--------------------------|-------------------------------------------------------------------------------------------------|
|**PostgreSQL 16**         |Hlavní databáze — scény, zařízení, harmonogramy, konfigurace                                     |
|**Redis 7**               |Live stav zařízení, pub-sub, session cache, distribuovaný lock                                   |
|**InfluxDB 2** (volitelné)|Time-series logování, telemetrie, dashboardy. Pokud není, loguje se do PostgreSQL tabulky `logs`.|
...
|**Turborepo**              |Monorepo build systém, sdílené balíčky|
|**pnpm**                   |Package manager (workspace podpora)   |
...
┌──────────▼──────────────────────────▼───────────────┐
│  API VRSTVA                                         │
│  Fastify HTTP Server + Socket.io WebSocket          │
└──────────────────────┬──────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────┐
│  VSTUPNÍ BUS                                        │
│  OSC Server (UDP)  │  TCP Input Server              │
└──────────────────────┬──────────────────────────────┘
                       │ EventBus.emit(...)
┌──────────────────────▼──────────────────────────────┐
...
┌─────────────────────────────────────────────────────┐
│  DATOVÁ VRSTVA                                      │
│  PostgreSQL │ Redis │ InfluxDB/SQLite (logy)        │
└─────────────────────────────────────────────────────┘
```
...
Strukturovaný audit log. Pokud je InfluxDB dostupné, preferuje se — tato tabulka slouží jako fallback.
...
Načte všechny aktivní `ScheduledJob` záznamy z DB při startu a registruje je do `node-cron`.
...
1. **PostgreSQL** — async insert do tabulky `logs` přes connection pool
1. **InfluxDB** (volitelné) — pokud je `INFLUXDB_URL` nastaveno
...
services:
  server:
    ...
    environment:
      ...
      - INFLUXDB_ORG=gallery
      - INFLUXDB_BUCKET=gallery_logs
    depends_on:
      postgres:
  ...
  influxdb:
    ...
    environment:
      ...
      DOCKER_INFLUXDB_INIT_ORG: gallery
      DOCKER_INFLUXDB_INIT_BUCKET: gallery_logs
...
# InfluxDB (volitelné — pokud není, logy jdou do PostgreSQL)
INFLUXDB_URL=http://localhost:8086
INFLUXDB_TOKEN=super_secret_token
INFLUXDB_PASSWORD=silne_heslo_influx
...
Přidat `apps/mcp-server/` jako samostatný Node.js proces. Komunikuje s `apps/server` výhradně přes HTTP API (volá `/api/v1/...`). Vystavuje tools pro LLM:
