...
describe("ExtronMatrixDriver", () => {
  ...
  // ── select-widget options (labels live on the connection, not the output) ──

  describe("select widget options", () => {
    test("12. every state carries {value,label} options built from the connection's input labels", async () => {
      const driver = new ExtronMatrixDriver();
      await driver.init(
        connConfig(mock.port, { inputCount: 3, inputs: ["Lectern", "Laptop"] }),
        testContext(),
      );
      await driver.connect();

      const result = await driver.executeCommand(endpoint(1), "setInput", { input: 1 });
      ...
      const state = await driver.readState(endpoint(1));
      expect(state.options).toEqual(result.state!.options as unknown[]);

      await driver.destroy();
    });

    test("13. no configured labels — options are just None + Input N up to inputCount", async () => {
      const driver = new ExtronMatrixDriver();
      await driver.init(connConfig(mock.port), testContext());
      await driver.connect();

      const state = await driver.readState(endpoint(1));
      const options = state.options as { value: number; label: string }[];
      expect(options).toHaveLength(11); // None + 10 (the mock's default inputCount)
      expect(options[0]).toEqual({ value: 0, label: "None" });
      expect(options[10]).toEqual({ value: 10, label: "Input 10" });

      await driver.destroy();
    });

    test("14. dry-run state also carries options", async () => {
      const driver = new ExtronMatrixDriver();
      await driver.init(
        connConfig(mock.port, { inputCount: 1, inputs: ["Lectern"] }),
        testContext(true),
      );
      await driver.connect();
