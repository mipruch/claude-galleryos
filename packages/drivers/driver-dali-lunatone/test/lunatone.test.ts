 ...
 * unknown-command, disconnect) plus the brightness-preservation behaviour that
 * used to live in server core (`redis/state.ts`'s `shouldPreserveBrightness`):
 * a DALI fixture reports `dimmable.status: 0` whenever it's physically off, but
 * the driver must remember the last non-zero level and restore it in `readState`
 * so a fader stays put across an off/on cycle — even across a driver restart,
 * since it's backed by the per-connection KV store, not an in-memory field.
 */
